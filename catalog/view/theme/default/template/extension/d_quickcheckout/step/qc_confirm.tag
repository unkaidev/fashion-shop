    ...
    <script>
        ...
        confirmCheckout(){
            if( getState().captcha_status == 1 && in_array( getAccount(), getState().config_captcha_page ) && getState().captcha_type == 'google'){
                if($('#payment_address_google_recaptcha').val() != $('textarea[name="g-recaptcha-response"]').val()){
                    $('#payment_address_google_recaptcha').val($('textarea[name="g-recaptcha-response"]').val());
                }
                this.store.dispatch('payment_address/update', $('#payment_address_google_recaptcha').serializeJSON());
        ...
        this.on("update", function(){
            //this.store.dispatch('payment_address/update', $('#payment_address_google_recaptcha').parents('form').serializeJSON().serializeJSON());
            tag.prev = pages[0] != this.store.getSession().page_id && this.store.getState().config.guest.confirm.buttons.prev.display == 1;
            tag.next = pages[pages.length-1] != this.store.getSession().page_id;
            tag.confirm = pages[pages.length-1] == this.store.getSession().page_id;
        });
